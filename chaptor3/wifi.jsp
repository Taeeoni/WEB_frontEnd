...
<html>
    ...
    <body>
		...
		<%
			//전체 데이터량을 구하기 위한 로직 
			ResultSet rset = stmt.executeQuery("select count(*) from freewifi;"); //총 데이터 몇개인지 세고 
			rset.next();
			...
			out.println("<tr><td colspan = 5><b>현재 페이지 : " + pageNum + "</b></td></tr>");
		%>
			<tr>
		...
		<%
			...
			rset = stmt.executeQuery("select * from freewifi;");//stmt로 freewifi DB를 불러와서 rset에다가 넣는다. 
			while (rset.next()) { //rset에 저장된 DB 내용 연속해서 매 줄 출력
			
			 if(lineCount > fromPT){ // fromPT 이상인 것부터 보여주게 하기 위한 if문 
				 out.println("<tr>");
				 out.println("<td width=50><p align=center>" + Integer.toString(lineCount) + "</p></td>"); //번호
				 out.println("<td width=600><p align=center>" + rset.getString(10) + "</p></td>"); //주소
				 out.println("<td width=50><p align=center>" + Double.toString(rset.getDouble(13)) + "</p></td>"); //위도
				 out.println("<td width=50><p align=center>" + Double.toString(rset.getDouble(14)) + "</p></td>"); //경도
				 
				 double dist = Math.sqrt( Math.pow(rset.getDouble(13) - lat, 2)
								+ Math.pow(rset.getDouble(14) - lng, 2)); // 거리계산 
				 
				 out.println("<td width=100><p align=center>" + dist + "</p></td>"); //거리
				 out.println("<tr>");
		...
		<%
			...
			if(!notShowBefore){ //왼쪽 꺽쇠를 띄우는 조건이라면 
				out.print("<td width=20><p align=center><a href='wifi.jsp?from=" 
