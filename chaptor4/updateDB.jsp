...
<html>
    ...
    <body>
        ...
        <%	
			...
			String ckey2 = request.getParameter("stu_name");
			String stu_name = new String(ckey2.getBytes("8859_1"), "utf-8");
		...
		<%
			...
			stmt.execute("update examtable set name ='" + stu_name 
			+"', studentid =" + ID +", kor=" + kor + ", eng=" + eng + ", mat="+ mat +" where studentid ="+ ID +";");
		...
		<%
			...
			rset = stmt.executeQuery("select * from examtable;");//stmt로 freewifi DB를 불러와서 rset에다가 넣는다. 
			while (rset.next()) { //rset에 저장된 DB 내용 연속해서 매 줄 출력
			
			 if(lineCount > fromPT){ // fromPT 이상인 것부터 보여주게 하기 위한 if문 
			 
			String tr_style = "<tr>"; // 변경된것과 아닌것을 구별하기 위한 tr 
			  
			 if(ID.equals(rset.getInt(2))){ //전체 테이블 중에 받아온 학번데이터와 같은 학번이 나온다면 노란색배경 
				 tr_style = "<tr class='yellow'>";
			 } 
			 out.println(tr_style);
